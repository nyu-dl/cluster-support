...
class SlurmExecutor(core.PicklingExecutor):
    ...
    def _submitit_command_str(self) -> str:
        ...
        python = os.getenv("MYPYTHON")
        if not python : python = f"{sys.executable}"
        return python + f" -u -m submitit.core._submit '{self.folder}'"
...
def _make_sbatch_string(
    ...
    lines += [
        ...
        f"srun --export=ALL --kill-on-bad-exit=1 --output '{stdout}' --error '{stderr}' --unbuffered {command}\n\n",
    ]
    return "\n".join(lines)